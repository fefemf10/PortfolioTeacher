...
namespace Portfolio.Application.Services.TeacherService
{
    public class TeacherProfessionalDevelopmentService : ITeacherProfessionalDevelopmentService
    {
        ...
        public TeacherProfessionalDevelopmentService(ApplicationContext db, IMapper mapper)
        {
            this.db = db;
            this.mapper = mapper;
        }
        ...
        public async Task<IEnumerable<ProfessionalDevelopment>> GetAllEntities(Guid id)
        {
            Teacher? teacher = await db.Teachers.Include(x => x.ProfessionalDevelopments).SingleOrDefaultAsync(x => x.Id == id) ?? throw new NotFoundByIdException();
            return teacher.ProfessionalDevelopments;
        }

        public async Task<IEnumerable<ShortItem>> GetShortAll(Guid id)
        {
            Teacher? teacher = await db.Teachers.Include(x => x.ProfessionalDevelopments).SingleOrDefaultAsync(x => x.Id == id) ?? throw new NotFoundByIdException();
            return mapper.Map<IEnumerable<ShortItem>>(teacher.ProfessionalDevelopments.ToList()).OrderBy(x => x.Year);
        }
